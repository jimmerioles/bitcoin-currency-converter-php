...
class ConvertUsingCoinbaseExchangeRatesTest extends TestCase
{
    public function test_can_convert_btc_to_fiat_currency_using_coinbase_exchange_rates()
    {
        //arrange
        ...
        //assert
        $this->assertEquals(1439.00, $result);
    }

    public function test_can_convert_fiat_currency_to_btc_using_coinbase_exchange_rates()
    {
        //arrange
        $mock = new MockHandler([new Response(200, ['Content-Type' => 'application/json'], $this->stubBody())]);
        $handler = HandlerStack::create($mock);
        ...
        //act
        $result = $convert->toBtc(200, 'USD');

        //assert
        $this->assertEquals(0.04586525, $result);
    }

    public function test_can_convert_btc_to_another_crypto_currency_using_coinbase_exchange_rates()
    {
        //arrange
        $mock = new MockHandler([new Response(200, ['Content-Type' => 'application/json'], $this->stubBody())]);
        $handler = HandlerStack::create($mock);
        ...
        //assert
        $this->assertEquals(23.30508475, $result);
    }

    public function test_can_convert_another_crypto_currency_to_btc_using_coinbase_exchange_rates()
    {
        //arrange
        $mock = new MockHandler([new Response(200, ['Content-Type' => 'application/json'], $this->stubBody())]);
        $handler = HandlerStack::create($mock);
        ...
        //act
        $result = $convert->toBtc(33.33333333, 'ETH');

        //assert
        $this->assertEquals(2.74433333, $result);
    }
