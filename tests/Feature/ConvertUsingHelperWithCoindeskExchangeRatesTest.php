...
{
    public function test_can_convert_btc_to_fiat_currency_using_coindesk_exchange_rates()
    {
    ...
    public function test_can_convert_fiat_currency_to_btc_using_coindesk_exchange_rates()
    {
        $mock = new MockHandler([new Response(200, ['Content-Type' => 'application/json'], $this->stubBody())]);
        $handler = HandlerStack::create($mock);
        $client = new Client(['handler' => $handler]);
